...
export class PlayerStatisticService {
	 ...
	 */
	async updatePlayerStatistic(player_id: string, playerEvent: PlayerEvent): Promise<[boolean, ServiceError[] | MongooseError]> {
		switch (playerEvent) {
			...
			default:
				return [null, [new ServiceError({ message: 'Event is not supported' })]];
		}
	}
	 ...
	 */
	private async trackPlayerMessageCount(player_id: string): Promise<[boolean, ServiceError[] | MongooseError]> {
