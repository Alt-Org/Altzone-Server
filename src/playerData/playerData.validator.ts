...
export default class PlayerDataValidator implements IValidator{
    public readonly validateCreate = [
        ...
        new Validator('currentCustomCharacterGameId', Location.BODY).ifProvided().isString().build(),
        new Validator('clanGameId', Location.BODY).ifProvided().isString().build(),
        new Validator('raidRoomGameId', Location.BODY).ifProvided().isString().build(),
