...
export function IsChatExists(validationOptions?: ValidationOptions) {
    return function (object: any, propertyName: string): void {
        registerValidationDecorator(IsChatExists.name, isChatExists, object, propertyName, validationOptions);
    };
}

@ValidatorConstraint({ name: isChatExists.name, async: true })
@Injectable()
export class isChatExists extends isEntityExists<Chat> implements ValidatorConstraintInterface{
    public constructor(@InjectModel(ModelName.CHAT) private readonly model: Model<Chat>) {
