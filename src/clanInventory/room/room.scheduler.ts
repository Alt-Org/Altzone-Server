  ...
  async removeInactiveRooms() {
    ...
    try {
      ...
      if (rooms && !roomsErrors) {
        ...
        const [soulHomes, soulHomesErrors] =
          await this.soulHomeService.basicService.readMany({
            filter: {
              _id: { $in: soulHomeIds },
            },
            session,
        ...
        const [stocks, stocksErrors] =
          await this.stockService.basicService.readMany({
            filter: {
              clan_id: { $in: clanIds },
            },
            session,
        ...
        for (const stock of stocks) {
          ...
          const roomIds = home
            ? homeRoomsMap.get(home._id.toString()) || []
            : [];
        ...
        if (itemBulk.length > 0) {
          const [, updateItemsErrors] =
            await this.itemService.basicService.bulkWrite(itemBulk, {
              session,
            });
          if (updateItemsErrors) throw new Error('Failed to update Items');
        }

        const [, deleteRoomsErrors] =
          await this.roomService.basicService.deleteMany({
            filter: {
