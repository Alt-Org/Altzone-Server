...
export class PlayerMocker {
  ...
  public getValid(): Partial<PlayerDto> {
    ...
    const newObj: Partial<PlayerDto> = {
      name,
      backpackCapacity,
      uniqueIdentifier,
    };
  ...
  public getWithoutFields(fieldsToEscape: string[]): Partial<PlayerDto> {
    ...
    return this.commonMocker.removeObjectFields<PlayerDto>(
      validObj,
      fieldsToEscape,
    );
  }
  public getWrongDT(
    ...
    const resp = this.commonMocker.generateWrongDataTypesResponse(
      validObj,
      fieldsToBeWrong,
    );
    const req = this.commonMocker.generateObjWithWrongDT(
      validObj,
      fieldsToBeWrong,
      options,
    );
  ...
  public getNotUnique(
    existingObj: object,
    notUniqueField: 'name' | 'uniqueIdentifier',
  ): object[] {
    const req = this.getValid();
    req['profile_id'] = existingObj['profile_id'];
    req[notUniqueField] = existingObj[notUniqueField];
    const resp = this.commonMocker.generateNotUniqueFieldsResponse({
      [notUniqueField]: existingObj[notUniqueField],
    });
    return [req, resp];
  }
  ...
  public getArrMeta(count: number) {
    return this.commonMocker.getMetadataForArray(
      'Player',
      ModelName.PLAYER,
      count,
    );
