...
export type GetWrongFieldDTOptions = {
  randStrings?: string[];
  min?: number;
  max?: number;
};
...
export class CommonMocker {
  public generateNotUniqueFieldsResponse(values: object) {
    ...
    for (const field in values) {
      errorArray.push(
        `Field '${field}' with value '${values[field]}' already exists`,
      );
    }
    return { statusCode: 409, message: errorArray, error: 'Conflict' };
  }

  public generateWrongDataTypesResponse<T = object>(
    rightObject: T,
    wrongFields?: string[],
  ) {
    if (wrongFields) {
  ...
  public removeObjectFields<T = any>(
    obj: T,
    fieldsToRemove: string[],
  ): Partial<T> {
    const result: Partial<T> = {};
    for (const field in obj) {
      if (!fieldsToRemove.includes(field)) result[field] = obj[field];
    }

    return result;
  }

  public generateObjWithWrongDT<T = object>(
    obj: T,
    fieldsToBeWrong?: string[],
    options?: GetWrongFieldDTOptions,
  ): unknown {
    if (fieldsToBeWrong) {
      for (let i = 0; i < fieldsToBeWrong.length; i++) {
        ...
        if (
          ...
          throw new Error(
            `CommonMocker.generateObjWithWrongDT(): could not generate value for '${field}' field, because it is a ${typeof field} or undefined`,
          );
      }
    ...
    for (const field in obj) {
      ...
      if (
        ...
        throw new Error(
          `CommonMocker.generateObjWithWrongDT(): could not generate value for ${field} field, because it is a ${typeof field}`,
        );
    }
  ...
  public getWrongFieldDataType(
    ...
    if (typeof fieldValue === 'boolean') {
      ...
      if (type === 'number') return this.getRandNum(options?.min, options?.max);
      else if (type === 'string')
        return this.getRandString(options?.randStrings);
    } else if (typeof fieldValue === 'number') {
      const type = this.chooseRandArrElem(dataTypesForNumber);
      if (type === 'boolean') return this.getRandBoolean();
      else if (type === 'string')
        return this.getRandString(options?.randStrings);
  ...
  public async cleanDB() {
    const db = await mongoose.connect(
      `mongodb://testUser:superSecretPassword@127.0.0.1:27017`,
      { dbName: 'altzone_test' },
    );
  ...
  private getWrongDTErrorString(field: string, value: any) {
    let valueError = '';
    if (typeof value === 'string') valueError = DataType.STRING;
    else if (typeof value === 'boolean') valueError = DataType.BOOLEAN;
