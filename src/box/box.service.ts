...
export class BoxService {
    ...
    async updateOneById(box: Partial<Box>) {
        ...
        const [isSuccess, errors] = await this.basicService.updateOneById(_id as any, fieldsToUpdate);
        if (errors && errors[0].reason === SEReason.NOT_UNIQUE && errors[0].field === 'adminPassword')
            errors[0].value = null;
