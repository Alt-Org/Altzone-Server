...
export class GroupAdminService {
     ...
     */
    public async isRegisteredPassword(passwordToCheck: string): Promise<IServiceReturn<boolean>> {
        if(!passwordToCheck)
            return [null, [
                new ServiceError({
                    reason: SEReason.REQUIRED, message: 'passwordToCheck parameter is required',
                    field: 'passwordToCheck', value: passwordToCheck
                })
            ]];

        const [adminInDB, errors] = await this.basicService.readOne({filter: {password: passwordToCheck}});
