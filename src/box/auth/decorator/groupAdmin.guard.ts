...
export class GroupAdminGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        const requiredAdmin = this.reflector.get(IS_GROUP_ADMIN, context.getHandler());
        if (!requiredAdmin) return true;

        const request = context.switchToHttp().getRequest<Request>();
        if (!request['user'])
            throw new APIError({
                reason: APIErrorReason.NOT_AUTHENTICATED, message: 'The user must be logged-in'
            });
        ...
        if (errors)
            throwAPIError(errors);

        if (!isAdmin)
            throwAPIError(new APIError({
