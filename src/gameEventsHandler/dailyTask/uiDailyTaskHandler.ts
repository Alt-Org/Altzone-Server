...
export default class UiDailyTaskHandler {
  ...
  async handleUIBasicTaskUpdate(
    ...
    const [[status, task], errors] = await this.taskService.updateTask(
      task_idStr,
      player_idStr,
      info.amount ?? 1,
    );
    ...
    if (errors)
      throw new ServiceError({
        ...errors[0],
        message: 'Failed to update UI daily task',
      });
    ...
    const [, deletionErrors] =
      await this.basicService.deleteOneById(task_idStr);

    if (deletionErrors)
      throw new ServiceError({
        ...deletionErrors[0],
        message: 'Failed to delete completed UI daily task',
      });
