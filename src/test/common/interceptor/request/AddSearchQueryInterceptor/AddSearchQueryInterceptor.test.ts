...
describe('AddSearchQueryInterceptor class test suite', () => {
    it('Should set a valid mongoFilter to request with query containing "AND"', async () => {
        const {context, nextFn, mockRequest} = createRequest({ search: 'name="John";AND;age>=30' });
        ...
        expect(mockRequest['mongoFilter']).toEqual({
            $and: expect.arrayContaining([{ age: { '$gte': 30 } }, { name: { '$eq': 'John' } }]),
        });
    });

    it('Should ignore fields not decorated with @Expose()', async () => {
        const {context, nextFn, mockRequest} = createRequest({ search: 'name="John";AND;salary>=30' });
    ...
    it('Should set a valid mongoFilter to request with query containing "OR"', async () => {
        const {context, nextFn, mockRequest} = createRequest({ search: 'name="John";OR;age>=30' });
    ...
    it('Should set a valid mongoFilter to request with query containing "OR" and "AND"', async () => {
        const {context, nextFn, mockRequest} = createRequest({ search: 'name="John";OR;age>=30;AND;size=20' });
        ...
        expect(mockRequest['mongoFilter']).toEqual({
            $or: expect.arrayContaining([
                { '$and': [{ name: { '$eq': 'John' } }] },
                { '$and': expect.arrayContaining([{ age: { '$gte': 30 } }, { size: { '$eq': 20 } }]) }
            ])
        });
    });
