...
describe('ProfileService.readOneWithAllCollections() test suite', () => {
    ...
    it('Should retrieve profile with all collections populated', async () => {
        const resp = await profileService.readOneWithAllCollections(existingProfile._id);
    ...
    it('Should return null if profile does not exist', async () => {
        const resp = await profileService.readOneWithAllCollections(getNonExisting_id());
        expect(resp).toBeNull();
    });

    it('Should throw MongooseError if _id is invalid', async () => {
        await expect(profileService.readOneWithAllCollections('invalid_id')).rejects.toThrow(MongooseError);
    });
});