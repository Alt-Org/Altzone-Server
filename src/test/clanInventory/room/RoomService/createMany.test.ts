...
describe('RoomService.createMany() test suite', () => {
    ...
    it('Should save rooms data to DB if input is valid', async () => {
        ...
        expect(clearedResp).toEqual(expect.arrayContaining([
            expect.objectContaining({...room1ToCreate, _id: expect.any(ObjectId), isActive: false}),
            expect.objectContaining({...room2ToCreate, _id: expect.any(ObjectId), isActive: false})
        ]));
    });

    it('Should return saved room data, if input is valid', async () => {
        const [result, errors] = await roomService.createMany([room1ToCreate, room2ToCreate]);
    ...
    it('Should return ServiceError with reason REQUIRED, if the provided cellCount is null', async () => {
        const invalidRoom = {...room1ToCreate, cellCount: null} as any;
        const [result, errors] = await roomService.createMany([invalidRoom, room2ToCreate]);
