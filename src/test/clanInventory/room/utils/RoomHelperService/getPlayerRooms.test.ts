...
describe('RoomHelperService.getPlayerRooms() test suite', () => {
    ...
    it('Should find existing rooms from DB', async () => {
        const [rooms, errors] = await roomHelperService.getPlayerRooms(existingPlayer._id);

        const clearedRooms = clearDBRespDefaultFields(rooms);

        expect(errors).toBeNull();
        expect(clearedRooms).toEqual(expect.objectContaining([existingRoom1, existingRoom2]));
    });

    it('Should return NOT_FOUND if player does not exists', async () => {
        const [rooms, errors] = await roomHelperService.getPlayerRooms(getNonExisting_id());
    ...
    it('Should return NOT_FOUND if player does not belong to any Clan', async () => {
        ...
        const [room, errors] = await roomHelperService.getPlayerRooms(existingPlayer._id);
