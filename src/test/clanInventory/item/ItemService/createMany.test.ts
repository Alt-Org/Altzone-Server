...
describe('ItemService.createMany() test suite', () => {
    ...
    it('Should save items data to DB if input is valid', async () => {
        ...
        expect(clearedResp).toEqual(expect.arrayContaining([
            expect.objectContaining({...item1ToCreate, _id: expect.any(ObjectId)}),
            expect.objectContaining({...item2ToCreate, _id: expect.any(ObjectId)})
        ]));
    });

    it('Should return saved items data, if input is valid', async () => {
        const [result, errors] = await itemService.createMany([item1ToCreate, item2ToCreate]);
    ...
    it('Should return ServiceError with reason REQUIRED, if the provided name is null', async () => {
        const invalidItem = {...item1ToCreate, name: null} as any;
        const [result, errors] = await itemService.createMany([invalidItem, item2ToCreate]);
