...
describe('ItemService.readMany() test suite', () => {
    ...
    it('Should return all items that match the provided filter', async () => {
        const [items, errors] = await itemService.readMany({ filter: allItemsFilter });
    ...
    it('Should return items with only specified fields in the options.select', async () => {
        ...
        const [items, errors] = await itemService.readMany({ filter: allItemsFilter, select });
    ...
    it('Should limit the number of returned items using options.limit', async () => {
        ...
        const [items, errors] = await itemService.readMany({ filter: allItemsFilter, limit });
    ...
    it('Should skip specified number of items using options.skip', async () => {
        ...
        const [items, errors] = await itemService.readMany({ filter: allItemsFilter, skip });
    ...
    it('Should return sorted items using options.sort', async () => {
        ...
        const [items, errors] = await itemService.readMany({ filter: allItemsFilter, sort });

        expect(errors).toBeNull();
        expect(items.map(items => items.name)).toEqual([item1.name, item2.name, item3.name]);
    });

    it('Should return items with reference objects using options.includeRefs', async () => {
        const [items, errors] = await itemService.readMany({ filter: allItemsFilter, includeRefs: [ModelName.STOCK] });
