 ...
 */
export const toContainSE_MORE_THAN_MAX: MatcherFunction<[object: any]> = function (object) {
    ...
    const isValid = object.find(item => isErrorWithReason(item, SEReason.MORE_THAN_MAX));

    return isValid ?
        returner.passTrue('Expected to not receive an array containing any ServiceErrors with reason MORE_THAN_MAX') :
        returner.passFalse('Expected to receive an array containing at least one ServiceError with reason MORE_THAN_MAX')
}
