 ...
 */
export const toContainSE_VALIDATION: MatcherFunction<[object: any]> = function (object) {
    ...
    const isValid = object.find(item => isErrorWithReason(item, SEReason.VALIDATION));

    return isValid ?
        returner.passTrue('Expected to not receive an array containing any ServiceErrors with reason VALIDATION') :
        returner.passFalse('Expected to receive an array containing at least one ServiceError with reason VALIDATION')
}
