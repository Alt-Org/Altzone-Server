...
describe('PlayerService.readOneWithAllCollections() test suite', () => {
    ...
    it('Should retrieve player with all collections populated', async () => {
        const resp = await playerService.readOneWithAllCollections(existingPlayer._id);
    ...
    it('Should return null if profile does not exist', async () => {
        const resp = await playerService.readOneWithAllCollections(getNonExisting_id());
        expect(resp).toBeNull();
    });

    it('Should throw MongooseError if _id is invalid', async () => {
        await expect(playerService.readOneWithAllCollections('invalid_id')).rejects.toThrow(MongooseError);
    });
});