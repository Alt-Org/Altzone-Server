...
describe('PlayerService.readWithCollections() test suite', () => {
    ...
    it('Should retrieve player with specified references', async () => {
        const resp = await playerService.readOneWithCollections(existingPlayer._id, ModelName.CLAN);
    ...
    it('Should retrieve player without references if withQuery is empty', async () => {
        const resp = await playerService.readOneWithCollections(existingPlayer._id, '');
    ...
    it('Should ignore non-existent references in withQuery', async () => {
        const resp = await playerService.readOneWithCollections(existingPlayer._id, 'NON_EXISTING');
        const data = resp['data']['Player'].toObject();
    ...
    it('Should return null if player does not exist', async () => {
        const resp = await playerService.readOneWithCollections(getNonExisting_id(), ModelName.CLAN);
        expect(resp).toBeNull();
    });

    it('Should throw MongooseError if _id is invalid', async () => {
        await expect(playerService.readOneWithCollections('invalid_id', ModelName.PLAYER)).rejects.toThrow(MongooseError);
    });
});