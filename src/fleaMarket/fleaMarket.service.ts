...
export class FleaMarketService {
	...
	private async handlePassedBuyVoting(voting: VotingDto, stockId: string) {
		...
		const newItem = await this.helperService.fleaMarketItemToCreateItemDto(item, stockId);
	...
	private async handleRejectedSellVoting(fmItemId: string, stockId: string) {
		const [fmItem, fmItemErrors] = await this.basicService.readOneById(
			fmItemId
		);
		...
		const item = await this.helperService.fleaMarketItemToCreateItemDto(fmItem, stockId);
