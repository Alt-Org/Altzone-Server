...
export function IsProfileExists(validationOptions?: ValidationOptions) {
    return function (object: any, propertyName: string): void {
        registerValidationDecorator(IsProfileExists.name, isProfileExists, object, propertyName, validationOptions);
    };
}

@ValidatorConstraint({ name: isProfileExists.name, async: true })
@Injectable()
export class isProfileExists extends isEntityExists<Profile> implements ValidatorConstraintInterface{
