...
export function IsUsernameExists(validationOptions?: ValidationOptions) {
    return function (object: any, propertyName: string): void {
        registerValidationDecorator(IsUsernameExists.name, isUsernameExists, object, propertyName, validationOptions);
    };
}

@ValidatorConstraint({ name: isUsernameExists.name, async: true })
@Injectable()
export class isUsernameExists extends isEntityExists<Profile> implements ValidatorConstraintInterface{
    public constructor(@InjectModel(ModelName.PROFILE) private readonly model: Model<Profile>) {
