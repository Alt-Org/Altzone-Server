  ...
  return (
    ...
    descriptor.value = function (this: any, ...args: any[]) {
      ...
      if (!areArgsValid(args))
        throw new Error(
          `The ${AddGetQueries.name} decorator needs Request object as the second argument of the method`,
        );

      const request = args[1];
      const { query } = request;
      if (!query) return originalMethod.apply(this, args);
      ...
      if (withQuery) {
        ...
        if (withRefs.length === 0) return originalMethod.apply(this, args);
...
function validateController(controller: any) {
  if (!controller)
    throw new Error(
      '"this" is undefined. Please check that it is accessible in the context',
    );
  ...
  if (
    ...
    !(
      ...
      controller.service.discriminators.includes(
        Discriminator.IConditionService,
      )
    )
  )
    throw new Error(
      'Field "service" (type IBasicService or IConditionService) is not defined in the controller class or it is wrong type. Please define one',
    );
