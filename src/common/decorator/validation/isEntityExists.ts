...
export class isEntityExists<T> implements ValidatorConstraintInterface {
  public constructor(
    ...
    searchField: string = '_id',
    entityModel?: Model<T>,
  ) {
    this.entityName = entityName;
    this.searchField = searchField;
    if (entityModel) this.entityModel = entityModel;
  }
  ...
  public async validate(
    ...
    if (!this.entityModel)
      throw new Error(
        `${isEntityExists.name} class ${validate.name}(): Can not validate entity existing. Model for DB query is not provided. Please provide the model for ${this.entityName} via constructor or setEntityModel(), before using the isEntityExists class`,
      );
    ...
    if (this.searchField.indexOf('_id') !== -1) {
      try {
        ...
        console.error(
          'Error occured on converting _id string to mongo object',
          e,
        );
