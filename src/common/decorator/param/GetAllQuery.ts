 ...
 * and convert it to the search options object, which can be used for mongoose queries
 */
export const GetAllQuery = createParamDecorator(
  (data: unknown, context: ExecutionContext): IGetAllQuery => {
    ...
    if (query['limit'] && limit < 1) limit = minLimit;
    if (query['limit'] && limit > maxLimit) limit = maxLimit;
