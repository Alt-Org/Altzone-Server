  ...
  >(originalConstructor: T) {
    return class extends originalConstructor implements IBasicService {
      ...
      public createOne = async (
        input: any,
      ): Promise<IResponseShape | MongooseError | null> => {
      ...
      public readOneById = async (
        ...
        _metaData?: string[],
      ): Promise<IResponseShape | null | MongooseError> => {
        const data = includeRefs
          ? await this.model.findById(_id).populate(includeRefs).exec()
          : await this.model.findById(_id).exec();
        if (this.readOnePostHook) this.readOnePostHook(data);
        return data ? this.configureResponse(data) : data;
      };

      public readOneWithCollections = async (
        _id: string,
        withQuery: string,
      ): Promise<IResponseShape | null | MongooseError> => {
      ...
      public readOneWithAllCollections = async (
        _id: string,
      ): Promise<IResponseShape | null | MongooseError> => {
      ...
      public readAll = async (
        ...
        if (query.select === null) return this.configureResponse([]);

        const { filter, select, ...searchOptions } = query;

        const data = await this.model
          .find(filter, null, searchOptions)
          .select(select);
        if (this.readAllPostHook) await this.readAllPostHook(data);
      ...
      public deleteOneById = async (
        _id: string,
        ignoreReferences?: IgnoreReferencesType,
      ): Promise<object | null | MongooseError> => {
      ...
      public deleteByCondition = async (
        condition: object,
        ...
        if (options?.isOne) {
          ...
          if (!entitiesToDelete || entitiesToDelete?.length === 0) return null;
      ...
      public getModelName = (): string =>
        this.modelName ? this.modelName : 'Model';
