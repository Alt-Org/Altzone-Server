 ...
 */
export class AddSortQueryInterceptor implements NestInterceptor {
  ...
  public intercept(
    ...
    if (!query || !query['sort']) return next.handle();

    const sortQuery = query['sort'] as string;
    const orderQuery =
      query['desc'] === '' || query['desc'] === 'true' ? -1 : 1;
    ...
    if (!possibleFields.includes(sortQuery)) return next.handle();
