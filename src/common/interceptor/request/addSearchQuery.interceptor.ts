...
export class AddSearchQueryInterceptor implements NestInterceptor {
  ...
  public intercept(
    ...
    if (!query || !query.search) return next.handle();
    ...
    if (searchParts.length % 2 === 0) return next.handle();

    //Get fields that can be queried
    const dtoClassInstance = new this.dtoClass();
    const possibleFields = Object.getOwnPropertyNames(
      instanceToPlain(dtoClassInstance),
    );
    ...
    for (let i = 0; i < andGroups.length; i++) {
      ...
      for (let i = 0; i < group.length; i += 2) {
        ...
        if (!query) break;
        andQuery.$and.push({
          [query.field]: { [query.selector]: query.value },
        });
      }
      if (andQuery.$and.length !== 0) andQueries.push(andQuery);
    }

    if (andQueries.length === 0) return next.handle();

    request['mongoFilter'] =
      andQueries.length === 1 ? andQueries[0] : { $or: andQueries };
...
function unpackSearchPair(
  ...
  if (isValueString && splitter !== '=') return null;

  let selector = queryToDB[splitter];
  if (!selector) return null;
  const parsedValue = isValueString
