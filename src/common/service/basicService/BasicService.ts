...
export default class BasicService implements IService {
  ...
  async readOneById<TOutput = any>(
    ...
    try {
      const { select, includeRefs, session } = options
        ? options
        : { select: undefined, includeRefs: [], session: undefined };

      const query = session
        ? this.model.findById(_id, select, { session })
        : this.model.findById(_id, select);
      const resp = await query.populate(includeRefs);
  ...
  if (!error)
    return [
      new ServiceError({
        ...
        message: 'Provided error is null',
      }),
    ];
  ...
  if (error?.name === 'StrictPopulateError' && error?.path)
    return [
      new ServiceError({
        ...
        message: `Reference "${error?.path}" is not in the DB schema`,
        field: 'includeRefs',
      }),
    ];
    ...
    if (fieldErrors.length === 0)
      return [
        new ServiceError({
          ...
          message: 'Could not convert this Mongoose validation error',
          additional: error,
        }),
      ];
  ...
  if (!error || !error.keyPattern)
    return [
      new ServiceError({
        reason: SEReason.UNEXPECTED,
        message:
          'Could not convert Mongoose unique field error to ServiceError',
        additional: error,
      }),
    ];
