...
function constraintToAPIErrorReason(constraint: string): APIErrorReason {
  ...
  if (constraint === 'isString') return APIErrorReason.NOT_STRING;

  if (constraint === 'notObject') return APIErrorReason.NOT_OBJECT;

  if (constraint === 'notDate') return APIErrorReason.NOT_DATE;

  if (constraint === 'isArray') return APIErrorReason.NOT_ARRAY;

  if (constraint === 'isBoolean') return APIErrorReason.NOT_BOOLEAN;

  if (constraint === 'isEnum') return APIErrorReason.WRONG_ENUM;

  if (moreThan.includes(constraint)) return APIErrorReason.MORE_THAN_MAX;

  if (lessThan.includes(constraint)) return APIErrorReason.LESS_THAN_MIN;
