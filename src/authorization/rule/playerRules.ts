...
export const playerRules: RulesSetterAsync<Ability, Subjects> = async (
  user,
  subject: any,
  action,
  subjectObj: any,
  ...
  if (action === Action.update) {
    ...
    if (subjectObj && reqPlayer_id && reqClan_id) {
      const clanWhereToAdd = await requestHelperService.getModelInstanceById(
        ModelName.CLAN,
        reqClan_id,
        ClanDto,
      );
      ...
      const playerToAdd = await requestHelperService.getModelInstanceById(
        ModelName.PLAYER,
        reqPlayer_id,
        PlayerDto,
      );
      ...
      if (playerToAdd.clan_id && playerToAdd.clan_id !== reqClan_id)
        throw new ForbiddenException(
          'Player is already in a clan. Please remove the Player from current clan first',
        );
      ...
      const playerToDelete = await requestHelperService.getModelInstanceById(
        ModelName.PLAYER,
        reqPlayer_id,
        PlayerDto,
      );
      ...
      const clanWhereFromDelete =
        await requestHelperService.getModelInstanceById(
          ModelName.CLAN,
          playerToDelete.clan_id,
          ClanDto,
        );
      ...
      if (!isAdmin && reqPlayer_id !== user.player_id)
        throw new ForbiddenException(
          'Logged-in user is not clan admin or user is trying to update other user profile',
        );
      //is last clan admin
      if (isLastAdmin)
        throw new ForbiddenException(
          'Player can not be removed from clan because it is the last clan admin. Please add another clan admin first',
        );
  ...
  if (action === Action.delete) {
    if (subjectObj._id !== user.player_id)
      throw new ForbiddenException(
        'The logged-in user can not delete other users profiles',
      );
    ...
    const playerClan = await requestHelperService.getModelInstanceById(
      ModelName.CLAN,
      playerToDelete.clan_id,
      ClanDto,
    );
    ...
    if (isLastAdmin)
      throw new ForbiddenException(
        'Player is the last clan admin. Please add another clan admin or remove the clan first',
      );
