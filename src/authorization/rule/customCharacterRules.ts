...
export const customCharacterRules: RulesSetterAsync<Ability, Subjects> = async (
  user,
  subject,
  action,
  subjectObj,
  ...
  if (action === Action.update || action === Action.delete) {
    const customCharacters = await getCustomCharacter(
      user,
      requestHelperService,
    );
    if (!customCharacters) {
...
const getCustomCharacter = async (
  user: User,
