...
import {swaggerDescription, swaggerTitle, swaggerVersion} from "./documentSetup/introSection";
...
export default class SwaggerSetuper {
    static setupSwaggerFromJSDocs(app: INestApplication) {
        ...
        for (const tag in swaggerTags)
            tagsToAdd.push(swaggerTags[tag]);
        ...
        SwaggerModule.setup(envVars.SWAGGER_PATH, app, documentFactory, swaggerUIOptions);
    }
}
