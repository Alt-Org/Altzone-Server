...
describe('CustomCharacterService.readOne() test suite', () => {
    ...
    const existingCharacter = characterBuilder.setSize(characterSize).setPlayerId(player_id).build();
    ...
    it('Should find and return a character based on the provided filter', async () => {
        ...
        expect(foundCharacter).toEqual(expect.objectContaining({ ...existingCharacter }));
    });
    ...
    it('Should return only specified fields in options.select array', async () => {
        ...
        const [foundCharacter, errors] = await characterService.readOne({ filter, select: ['size', '_id'] });
    ...
    it('Should return character with reference objects specified in options.includeRefs array', async () => {
        ...
        await characterModel.findByIdAndUpdate(existingCharacter._id, { player_id: player._id });

        const [foundCharacter, errors] = await characterService.readOne({ filter, includeRefs: [ModelName.PLAYER] });
        const clearedCharacter = clearDBRespDefaultFields(foundCharacter);

        expect(errors).toBeNull();
        expect(clearedCharacter.Player).toEqual(expect.objectContaining({
