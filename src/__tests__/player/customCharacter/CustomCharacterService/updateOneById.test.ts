...
describe('CustomCharacter.updateOneById() test suite', () => {
  ...
  const characterUpdateBuilder = PlayerBuilderFactory.getBuilder(
  ...
  const existingCharacter = characterBuilder
    .setPlayerId(new ObjectId(getNonExisting_id()))
    .setSize(10)
    .build();
  ...
  it('Should update character in the DB and return true if the input is valid', async () => {
    ...
    const updateData = characterUpdateBuilder
      .setId(existingCharacter._id)
      .setSize(updatedSize)
      .build();

    const [wasUpdated, errors] =
      await characterService.updateOneById(updateData);
