...
describe('CustomCharacterService.readPlayerBattleCharacters() test suite', () => {
  ...
  beforeEach(async () => {
    ...
    player1Character1 = characterBuilder
      .setSize(10)
      .setPlayerId(player1._id)
      .build();
    player1Character2 = characterBuilder
      .setSize(20)
      .setPlayerId(player1._id)
      .build();
    player1Character3 = characterBuilder
      .setSize(30)
      .setPlayerId(player1._id)
      .build();
    ...
    const player1Characters = playerUpdateBuilder
      .setBattleCharacterIds([
  ...
  it('Should ignore characters that does not belong to the player', async () => {
    ...
    const updateData = playerUpdateBuilder
      .setBattleCharacterIds([player1Character1._id, otherPlayerCharacter])
      .build();
  ...
  it('Should ignore characters that does not exists', async () => {
    ...
    const updateData = playerUpdateBuilder
      .setBattleCharacterIds([player1Character1._id, nonExistingCharacter])
      .build();
    await playerModel.findByIdAndUpdate(player1._id, updateData);
  ...
  it('Should return ServiceError NOT_FOUND if no characters are found', async () => {
    const updateData = playerUpdateBuilder
      .setBattleCharacterIds([getNonExisting_id(), getNonExisting_id()])
      .build();
    await playerModel.findByIdAndUpdate(player1._id, updateData);
  ...
  it('Should return ServiceError REQUIRED if player _id is null', async () => {
    const [characters, errors] =
      await characterService.readPlayerBattleCharacters(null);
  ...
  it('Should return ServiceError REQUIRED if player _id is undefined', async () => {
    const [characters, errors] =
      await characterService.readPlayerBattleCharacters(undefined);
