...
describe('PlayerService.getAll() test suite', () => {
    ...
    beforeEach(async () => {
        ...
        const playerToCreate1 = playerBuilder.setName(playerName1).setUniqueIdentifier(playerName1).build();
        const playerResp1 = await playerModel.create(playerToCreate1);
        player1 = playerResp1.toObject();

        const playerToCreate2 = playerBuilder.setName(playerName2).setUniqueIdentifier(playerName2).build();
    ...
    it('Should return NOT_FOUND ServiceError for non-existing player', async () => {
        const [players, errors] = await playerService.getAll({filter: {name: 'non-existing'}});
    ...
    it('Should return only specified in select option fields', async () => {
        const [players, errors] = await playerService.getAll({select: ['_id', 'name']});
    ...
    it('Should return desc sorted by name players if requested', async () => {
        const [players, errors] = await playerService.getAll({sort: {name: -1}});
    ...
    it('Should be able to skip first found player in DB if requested', async () => {
        const [players, errors] = await playerService.getAll({skip: 1, sort: {name: -1}});
    ...
    it('Should include get players\' clan if requested', async () => {
        const [players, errors] = await playerService.getAll({includeRefs: [ModelName.CLAN]});
    ...
    it('Should ignore non-existing references requested', async () => {
        ...
        const [players, errors] = await playerService.getAll({includeRefs: [nonExistingRef]});
