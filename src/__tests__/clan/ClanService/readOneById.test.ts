...
describe('ClanService.readOneById() test suite', () => {
    ...
    it('Should return only requested in "select" fields', async () => {
        const [clan, errors] = await clanService.readOneById(existingClan._id, { select: [ '_id', 'name' ] });
    ...
    it('Should get clan collection references if they exists in DB', async () => {
        const [clan, errors] = await clanService.readOneById(existingClan._id, { includeRefs: [ ModelName.PLAYER ] });
    ...
    it('Should ignore non-existing schema references requested', async () => {
        ...
        const [clan, errors] = await clanService.readOneById(existingClan._id, { includeRefs: nonExistingReferences });
