...
describe('JoinService.removePlayerFromClan() test suite', () => {
    ...
    it('Should throw NotFoundException if clan does not exists', async () => {
        ...
        await expect(
            joinService.removePlayerFromClan(player._id, nonExisting_id)
    ...
    it('Should throw NotFoundException if player with that _id does not exists', async () => {
        ...
        await expect(joinService.removePlayerFromClan(nonExisting_id, clan._id)).rejects.toThrow(NotFoundException);
    });

    it('Should remove clan if the last player is removed', async () => {
        await joinService.removePlayerFromClan(player._id, clan._id);
    ...
    it('Should decrease playerCount field if player is removed', async () => {
