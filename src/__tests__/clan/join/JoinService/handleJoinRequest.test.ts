...
describe('JoinService.handleJoinRequest() test suite', () => {
    ...
    it('Should create a clan join request in DB if input is valid', async () => {
        const joinToCreate = joinBuilder.setClanId(closedClan._id).setPlayerId(player._id).build();

        await joinService.handleJoinRequest(joinToCreate);

        const dbData = await joinModel.findOne({clan_id: closedClan._id, player_id: player._id});
    ...
    it('Should throw NotFoundException if clan with that _id does not exists', async () => {
        ...
        const joinToCreate = joinBuilder.setClanId(nonExisting_id).setPlayerId(player._id).build();

        await expect(joinService.handleJoinRequest(joinToCreate)).rejects.toThrow(NotFoundException);
    });
    ...
    it('Should throw NotFoundException if player with that _id does not exists', async () => {
        const nonExisting_id = getNonExisting_id();
        const joinToCreate = joinBuilder.setClanId(openClan._id).setPlayerId(nonExisting_id).build();
    ...
    it('Should throw BadRequestException if join request to closed clan is without message', async () => {
        const joinToCreate = joinBuilder.setClanId(closedClan._id).setPlayerId(player._id).setJoinMessage(undefined).build();

        await expect(joinService.handleJoinRequest(joinToCreate)).rejects.toThrow(BadRequestException);
    });
    ...
    it('Should remove clan if the last player of it make a join request to another open clan', async () => {
        ...
        const joinToCreate = joinBuilder.setClanId(openClan._id).setPlayerId(player._id).build();
        await joinService.handleJoinRequest(joinToCreate);
    ...
    it('Should decrease playerCount field of an open clan if player joins another clan', async () => {
        const oldClanToCreate = clanBuilder.setName('oldClan').setPlayerCount(2).build();
        const oldClan = await clanModel.create(oldClanToCreate);
        await playerModel.updateOne({_id: player._id}, { clan_id: oldClan._id });
        const anotherClanMember = playerBuilder.setClanId(oldClan._id)
            .setName('player2')
