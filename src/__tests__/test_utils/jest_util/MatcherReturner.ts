...
export default class MatcherReturner {
  ...
  constructor({
    ...
    received,
    utils,
  }: { expected?: any; received?: any; utils?: any } = {}) {
  ...
  passFalse(
    message: string,
    options?: { onlyMessage?: boolean; expected?: any },
  ) {
   ...
   * @returns {string}
   */
  private determineMessageToPrint(
    message: string,
    options?: { onlyMessage?: boolean; expected?: any },
  ) {
    if ((options && options.onlyMessage) || !this.utils) return message;
