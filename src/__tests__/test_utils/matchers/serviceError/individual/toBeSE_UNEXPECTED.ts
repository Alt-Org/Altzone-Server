 ...
 */
export const toBeSE_UNEXPECTED: MatcherFunction<[object: any]> = function (
  object,
) {
  ...
  return isValid
    ? returner.passTrue(
        'Expected to not receive a ServiceError with reason UNEXPECTED',
      )
    : returner.passFalse(
        'Expected to receive ServiceError with reason UNEXPECTED',
      );
};
