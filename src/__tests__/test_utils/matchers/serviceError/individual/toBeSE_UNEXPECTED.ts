 ...
 */
export const toBeSE_UNEXPECTED: MatcherFunction<[object: any]> = function(object) {
    ...
    const isValid = isErrorWithReason(object, SEReason.UNEXPECTED);

    return isValid ?
        returner.passTrue('Expected to not receive a ServiceError with reason UNEXPECTED') :
        returner.passFalse('Expected to receive ServiceError with reason UNEXPECTED');
}
