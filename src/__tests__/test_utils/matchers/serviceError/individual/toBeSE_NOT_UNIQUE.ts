 ...
 */
export const toBeSE_NOT_UNIQUE: MatcherFunction<[object: any]> = function (
  object,
) {
  ...
  return isValid
    ? returner.passTrue(
        'Expected to not receive a ServiceError with reason NOT_UNIQUE',
      )
    : returner.passFalse(
        'Expected to receive ServiceError with reason NOT_UNIQUE',
      );
};
