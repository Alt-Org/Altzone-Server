 ...
 */
export const toBeSE: MatcherFunction<[expected: SEReason]> = function (received, expected) {
    ...
    return isValid ?
        returner.passTrue(`Expected to not receive a ServiceError with reason ${expected}`) :
        returner.passFalse(`Expected to receive ServiceError with reason ${expected}`);
}
