 ...
 */
export const toBeSE_VALIDATION: MatcherFunction<[object: any]> = function (
  object,
) {
  ...
  return isValid
    ? returner.passTrue(
        'Expected to not receive a ServiceError with reason VALIDATION',
      )
    : returner.passFalse(
        'Expected to receive ServiceError with reason VALIDATION',
      );
};
