 ...
 */
export const toBeSE_NOT_OBJECT: MatcherFunction<[object: any]> = function (
  object,
) {
  ...
  return isValid
    ? returner.passTrue(
        'Expected to not receive a ServiceError with reason NOT_OBJECT',
      )
    : returner.passFalse(
        'Expected to receive ServiceError with reason NOT_OBJECT',
      );
};
