 ...
 */
export const toBeSE_WRONG_ENUM: MatcherFunction<[object: any]> = function (
  object,
) {
  ...
  return isValid
    ? returner.passTrue(
        'Expected to not receive a ServiceError with reason WRONG_ENUM',
      )
    : returner.passFalse(
        'Expected to receive ServiceError with reason WRONG_ENUM',
      );
};
