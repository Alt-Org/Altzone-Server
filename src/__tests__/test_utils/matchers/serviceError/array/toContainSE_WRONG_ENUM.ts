 ...
 */
export const toContainSE_WRONG_ENUM: MatcherFunction<[object: any]> = function (
  object,
) {
  ...
  const isValid = object.find((item) =>
    isErrorWithReason(item, SEReason.WRONG_ENUM),
  );

  return isValid
    ? returner.passTrue(
        'Expected to not receive an array containing any ServiceErrors with reason WRONG_ENUM',
      )
    : returner.passFalse(
        'Expected to receive an array containing at least one ServiceError with reason WRONG_ENUM',
      );
};
