 ...
 */
export const toContainSE_NOT_BOOLEAN: MatcherFunction<[object: any]> =
  function (object) {
    ...
    const isValid = object.find((item) =>
      isErrorWithReason(item, SEReason.NOT_BOOLEAN),
    );

    return isValid
      ? returner.passTrue(
          'Expected to not receive an array containing any ServiceErrors with reason NOT_BOOLEAN',
        )
      : returner.passFalse(
          'Expected to receive an array containing at least one ServiceError with reason NOT_BOOLEAN',
        );
  };
