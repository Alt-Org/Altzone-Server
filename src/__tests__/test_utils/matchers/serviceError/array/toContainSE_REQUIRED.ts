 ...
 */
export const toContainSE_REQUIRED: MatcherFunction<[object: any]> = function (
  object,
) {
  ...
  const isValid = object.find((item) =>
    isErrorWithReason(item, SEReason.REQUIRED),
  );

  return isValid
    ? returner.passTrue(
        'Expected to not receive an array containing any ServiceErrors with reason REQUIRED',
      )
    : returner.passFalse(
        'Expected to receive an array containing at least one ServiceError with reason REQUIRED',
      );
};
