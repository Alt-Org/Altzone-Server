 ...
 */
export const toContainSE_LESS_THAN_MIN: MatcherFunction<[object: any]> =
  function (object) {
    ...
    const isValid = object.find((item) =>
      isErrorWithReason(item, SEReason.LESS_THAN_MIN),
    );

    return isValid
      ? returner.passTrue(
          'Expected to not receive an array containing any ServiceErrors with reason LESS_THAN_MIN',
        )
      : returner.passFalse(
          'Expected to receive an array containing at least one ServiceError with reason LESS_THAN_MIN',
        );
  };
