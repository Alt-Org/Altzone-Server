 ...
 */
export const toContainSE_UNEXPECTED: MatcherFunction<[object: any]> = function (object) {
    ...
    const isValid = object.find(item => isErrorWithReason(item, SEReason.UNEXPECTED));

    return isValid ?
        returner.passTrue('Expected to not receive an array containing any ServiceErrors with reason UNEXPECTED') :
        returner.passFalse('Expected to receive an array containing at least one ServiceError with reason UNEXPECTED')
}
