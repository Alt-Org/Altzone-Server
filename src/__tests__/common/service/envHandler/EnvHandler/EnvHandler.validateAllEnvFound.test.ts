...
describe('EnvHandler.validateAllEnvFound() test suite', () => {
  ...
  it('Should throw ServiceError MISCONFIGURED if any of the env vars can not be found from .env', () => {
    ...
    expect(notDefinedVar).toThrow(
      expect.objectContaining({
        reason: SEReason.MISCONFIGURED,
        message:
          'Can not start the API. One or more of the environment variables is missing from .env file',
        additional: [
          new ServiceError({
            reason: SEReason.MISCONFIGURED,
            message:
              'EnvHandler: The environment variable PORT is missing or undefined in the .env file. Please add it to the .env file and restart the API',
            field: 'PORT',
            value: undefined,
          }),
        ],
