...
import {APIError, convertToAPIError} from "../../../../../common/controller/APIError";
...
describe('FormatAPIResponseInterceptor.intercept() test suite', () => {
    ...
    it('Should throw APIError from tuple [null, error]', async () => {
        const apiError = new APIError({ message: 'bad', reason: APIErrorReason.BAD_REQUEST });

        (callHandler.handle as jest.Mock).mockReturnValue(of([null, apiError]));

        await expect(interceptor.intercept(executionContext, callHandler).toPromise()).rejects.toThrow(HttpException);
    ...
    it('Should handle async resolved tuple [null, APIError]', async () => {
        const apiError = new APIError({ message: 'async error', reason: APIErrorReason.NOT_AUTHORIZED });

        (callHandler.handle as jest.Mock).mockReturnValue(of(Promise.resolve([null, apiError])));
