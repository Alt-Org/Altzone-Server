...
describe('SoulHomeService.updateOneById() test suite', () => {
  ...
  const soulHomeUpdateBuilder =
    ClanInventoryBuilderFactory.getBuilder('UpdateSoulHomeDto');
  ...
  it('Should update soul home in the DB and return true if the input is valid', async () => {
    ...
    const updateData = soulHomeUpdateBuilder
      .setId(existingSoulHome._id)
      .setName(updatedName)
      .build();

    const [wasUpdated, errors] =
      await soulHomeService.updateOneById(updateData);
  ...
  it('Should throw error if input is null or undefined', async () => {
    ...
    const undefinedInput = async () =>
      await soulHomeService.updateOneById(undefined);
