...
describe('StockService.updateStockCellCount() test suite', () => {
    ...
    const cellCount = 20;
    const existingStock = stockBuilder.setClanId(new ObjectId(getNonExisting_id())).setCellCount(cellCount).build();
    ...
    it('Should increase cellCount in the DB and return true if the amount is positive', async () => {
        ...
        const [wasUpdated, errors] = await stockService.updateStockCellCount(existingStock._id, increaseAmount);
    ...
    it('Should decrease cellCount in the DB and return true if the amount is negative', async () => {
        ...
        const [wasUpdated, errors] = await stockService.updateStockCellCount(existingStock._id, decreaseAmount);
    ...
    it('Should return ServiceError NOT_FOUND if the stock with provided _id does not exist', async () => {
        const [wasUpdated, errors] = await stockService.updateStockCellCount(getNonExisting_id(), 10);
