...
describe('StockService.readOneById() test suite', () => {
    ...
    let existingItem = itemBuilder.build();
    ...
    it('Should return only requested in "select" fields', async () => {
        const [stock, errors] = await stockService.readOneById(existingStock._id, { select: [ '_id', 'cellCount' ] });

        const clearedStock = clearDBRespDefaultFields(stock);
        const expected = { _id: existingStock._id, cellCount: existingStock.cellCount };
    ...
    it('Should get stock collection references if they exists in DB', async () => {
        const [stock, errors] = await stockService.readOneById(existingStock._id, { includeRefs: [ ModelName.ITEM ] });
    ...
    it('Should ignore non-existing schema references requested', async () => {
        ...
        const [stock, errors] = await stockService.readOneById(existingStock._id, { includeRefs: nonExistingReferences });
