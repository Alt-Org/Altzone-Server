...
describe('StockService.readAll() test suite', () => {
  ...
  it('Should return stocks with only specified fields using options.select', async () => {
    ...
    expect(clearedClans).toEqual([
      expect.objectContaining({
        cellCount: stock1.cellCount,
        _id: expect.any(ObjectId),
      }),
      expect.objectContaining({
        cellCount: stock2.cellCount,
        _id: expect.any(ObjectId),
      }),
    ]);
  });
  ...
  it('Should return stocks with reference objects using options.includeRefs', async () => {
    ...
    expect(clearedItem1).toEqual(
      expect.objectContaining({ ...existingItem1, _id: expect.any(ObjectId) }),
    );
    expect(clearedItem2).toEqual(
      expect.objectContaining({ ...existingItem2, _id: expect.any(ObjectId) }),
    );
  });
