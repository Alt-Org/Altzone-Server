...
describe('ItemService.updateOneById() test suite', () => {
  ...
  const itemUpdateBuilder =
    ClanInventoryBuilderFactory.getBuilder('UpdateItemDto');
  const itemModel = ItemModule.getItemModel();
  const existingItem = itemBuilder
    .setRoomId(new ObjectId(getNonExisting_id()))
  ...
  it('Should update item in the DB and return true if the input is valid', async () => {
    ...
    const updateData = itemUpdateBuilder
  ...
  it('Should throw error if input is null or undefined', async () => {
    ...
    const undefinedInput = async () =>
      await itemService.updateOneById(undefined);
