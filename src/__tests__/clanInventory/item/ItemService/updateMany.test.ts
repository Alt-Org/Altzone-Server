...
describe('ItemService.updateMany() test suite', () => {
  ...
  it('Should update all items that match the provided filter', async () => {
    ...
    const [wasUpdated, errors] = await itemService.updateMany(updateData, {
  ...
  it('Should not throw any error if input is null or undefined', async () => {
    const nullInput = async () =>
      await itemService.updateMany(null, { filter: allItemsFilter });
    const undefinedInput = async () =>
      await itemService.updateMany(undefined, { filter: allItemsFilter });
