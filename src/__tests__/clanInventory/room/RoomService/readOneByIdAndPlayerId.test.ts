...
describe('Room.readOneByIdAndPlayerId() test suite', () => {
  ...
  it('Should not throw if provided room _id is null or undefined', async () => {
    const null_idCall = async () =>
      await roomService.readOneByIdAndPlayerId(null, existingPlayer._id);
    const undefined_idCall = async () =>
      await roomService.readOneByIdAndPlayerId(undefined, existingPlayer._id);
  ...
  it('Should return NOT_FOUND if provided room _id is null or undefined', async () => {
    const [roomNullCall, errorsNullCall] =
      await roomService.readOneByIdAndPlayerId(null, existingPlayer._id);
    const [roomUndefinedCall, errorsCall] =
      await roomService.readOneByIdAndPlayerId(undefined, existingPlayer._id);
