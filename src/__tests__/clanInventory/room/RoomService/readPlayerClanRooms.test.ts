...
describe('Room.readPlayerClanRooms() test suite', () => {
    ...
    it('Should find existing rooms from DB', async () => {
        const [room, errors] = await roomService.readPlayerClanRooms(existingPlayer._id, {filter: {}});

        const clearedRoom = clearDBRespDefaultFields(room);

        expect(errors).toBeNull();
        expect(clearedRoom).toEqual(expect.objectContaining([existingRoom1, existingRoom2]));
    });

    it('Should return NOT_FOUND if player does not exists', async () => {
        const [room, errors] = await roomService.readPlayerClanRooms(getNonExisting_id());
    ...
    it('Should return NOT_FOUND if player does not belong to any Clan', async () => {
        ...
        const [room, errors] = await roomService.readPlayerClanRooms(existingPlayer._id);
