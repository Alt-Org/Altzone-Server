...
describe('ProfileService.readWithCollections() test suite', () => {
    ...
    it('Should retrieve profile with specified references', async () => {
        const resp = await profileService.readOneWithCollections(existingProfile._id, ModelName.PLAYER);
    ...
    it('Should retrieve profile without references if withQuery is empty', async () => {
        const resp = await profileService.readOneWithCollections(existingProfile._id, '');
    ...
    it('Should ignore non-existent references in withQuery', async () => {
        const resp = await profileService.readOneWithCollections(existingProfile._id, 'NON_EXISTING');
        const data = resp['data']['Profile'].toObject();
    ...
    it('Should return null if profile does not exist', async () => {
        const resp = await profileService.readOneWithCollections(getNonExisting_id(), ModelName.PLAYER);
        expect(resp).toBeNull();
    });

    it('Should throw MongooseError if _id is invalid', async () => {
        await expect(profileService.readOneWithCollections('invalid_id', ModelName.PLAYER)).rejects.toThrow(MongooseError);
    });
});