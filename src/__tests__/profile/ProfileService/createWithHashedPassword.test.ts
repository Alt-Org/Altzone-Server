...
describe('ProfileService.createWithHashedPassword() test suite', () => {
  ...
  it('Should return created profile with a hashed password', async () => {
    const [profile, errors] = (await profileService.createWithHashedPassword(
      createProfileData,
    )) as any;
    ...
    expect(
      await argon2.verify(profile.password, createProfileData.password),
    ).toBe(true);
  });
  ...
  it('Should create profile in DB with a hashed password', async () => {
    ...
    const dbResp = await profileModel.findOne({
      username: createProfileData.username,
    });

    expect(dbResp.username).toBe(createProfileData.username);
    expect(
      await argon2.verify(dbResp.password, createProfileData.password),
    ).toBe(true);
  });
