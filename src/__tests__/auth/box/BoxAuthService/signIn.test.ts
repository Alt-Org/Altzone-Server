...
describe('BoxAuthService.signIn() test suite', () => {
    ...
    const signAsyncSpy = jest.spyOn(JwtService.prototype, 'signAsync').mockResolvedValue(accessToken);
    jest.spyOn(JwtService.prototype, 'decode').mockReturnValue({exp: tokenExpires});
    ...
    beforeEach(async () => {
        ...
        existingBox = boxBuilder
            ...
            .setAdminPlayerId(new ObjectId()).setAdminProfileId(new ObjectId(existingAdminProfile._id))
    ...
    it('Should return access token, expiration time, profile, player and clan data if username and password are valid', async () => {
        ...
        const player = playerBuilder.setProfileId(existingAdminProfile._id).setClanId(createdClanDB._id).build();
        ...
        expect(clearedResult).toEqual({...expectedResult, Player: expect.anything(), Clan: expect.anything()});
    ...
    it('Should call signAsync() with payload containing groupAdmin set to false if the profile is not group admin', async () => {
        ...
        await boxModel.findByIdAndUpdate(existingBox._id, {
            ...
            testers: [ { profile_id: existingAdminProfile._id, player_id: createdPlayer._id, isClaimed: true } ]
        });
    ...
    it('Should return null if profile does not exists', async () => {
        const result = await boxAuthService.signIn('non-existing-user', validPassword);
    ...
    it('Should return null if password is not valid', async () => {
        const result = await boxAuthService.signIn(validUsername, 'invalid-password');
        expect(result).toBeNull();
    });
