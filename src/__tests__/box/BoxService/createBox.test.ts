...
describe('BoxService.createOne() test suite', () => {
    ...
    const adminPlayer = playerBuilder.setName(boxAdmin).setUniqueIdentifier(boxAdmin).build();
    const testerPlayer1 = playerBuilder.setName(testerName1).setUniqueIdentifier(testerName1).build();
    const testerPlayer2 = playerBuilder.setName(testerName2).setUniqueIdentifier(testerName2).build();
    ...
    beforeEach(async () => {
        ...
        validBox = boxBuilder
            .setAdminPassword(existingAdmin.password).setAdminPlayerId(new ObjectId(adminPlayer._id)).setAdminProfileId(new ObjectId(adminProfile._id))
            .setTesters([
                { profile_id: new ObjectId(testerProfile1._id), player_id: new ObjectId(testerPlayer1._id), isClaimed: false },
                { profile_id: new ObjectId(testerProfile2._id), player_id: new ObjectId(testerPlayer2._id), isClaimed: false }
            ])
            ...
            .setRoomIds([
                new ObjectId(existingRoom11._id), new ObjectId(existingRoom12._id),
                new ObjectId(existingRoom21._id), new ObjectId(existingRoom22._id)
            ])
    ...
    it('Should save box data to DB if input is valid', async () => {
        ...
        expect(clearedResp).toEqual(expect.objectContaining({...validBox, _id: expect.any(ObjectId)}));
    });
    ...
    it('Should return saved box data, if input is valid', async () => {
        ...
        expect(result).toEqual(expect.objectContaining({...validBox, _id: expect.any(ObjectId)}));
    });
