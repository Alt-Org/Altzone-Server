...
describe('BoxService.deleteBoxReferences() test suite', () => {
    ...
    beforeEach(async () => {
        ...
        const testerPlayer1 = playerBuilder
            .setName(testerName1).setUniqueIdentifier(testerName1)
            .setClanId(existingClan1._id).setProfileId(testerProfile1._id).build();
        ...
        const testerPlayer2 = playerBuilder
            .setName(testerName2).setUniqueIdentifier(testerName2)
            .setClanId(existingClan1._id).setProfileId(testerProfile2._id).build();
        const testerPlayerResp2 = await playerModel.create(testerPlayer2);
        testerPlayer2._id = testerPlayerResp2._id;
        const tester1 = testerBuilder.setProfileId(new ObjectId(testerProfile1._id)).setPlayerId(new ObjectId(testerPlayer1._id)).build();
        const tester2 = testerBuilder.setProfileId(new ObjectId(testerProfile2._id)).setPlayerId(new ObjectId(testerPlayer2._id)).build();
    ...
    it("Should return true if the box references were removed", async () => {
        const [isRemoved, errors] = await boxService.deleteBoxReferences(existingBox);
    ...
    it("Should delete admin group player from DB", async () => {
        ...
        const adminPlayerInDB = await playerModel.find({ _id: existingBox.adminPlayer_id });
    ...
    it("Should delete admin group profile from DB", async () => {
        ...
        const profileInDB = await profileModel.find({ _id: existingBox.adminProfile_id });
    ...
    it("Should delete all related soul homes from DB", async () => {
        ...
        const soulHomesInDB = await soulHomeModel.find({ _id: { $in: existingBox.soulHome_ids } });
    ...
    it("Should delete all related rooms from DB", async () => {
        ...
        const roomsInDB = await roomModel.find({ _id: { $in: existingBox.room_ids } });
    ...
    it("Should delete all related stocks from DB", async () => {
        ...
        const stocksInDB = await stockModel.find({ _id: { $in: existingBox.stock_ids } });
        expect(stocksInDB).toHaveLength(0);
    });

    it("Should delete the related chat from DB", async () => {
        await boxService.deleteBoxReferences(existingBox);
    ...
    it("Should delete all related testers profiles from DB", async () => {
        await boxService.deleteBoxReferences(existingBox);

        const testerIds = existingBox.testers.map(tester => tester.profile_id);
        const testersInDB = await profileModel.find({ _id: { $in: testerIds } });
        expect(testersInDB).toHaveLength(0);
    });

    it("Should delete all related testers players from DB", async () => {
        await boxService.deleteBoxReferences(existingBox);

        const testerIds = existingBox.testers.map(tester => tester.player_id);
        const testersInDB = await profileModel.find({ _id: { $in: testerIds } });
        expect(testersInDB).toHaveLength(0);
    });
});