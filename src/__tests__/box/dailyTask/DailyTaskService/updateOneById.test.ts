...
describe('DailyTaskService.updateOneById() test suite', () => {
  ...
  it('Should update task in the DB and return true if the input is valid', async () => {
    ...
    const updateData = taskBuilder
      .setId(existingTask._id.toString())
      .setAmount(updatedAmount)
      .build();

    const [wasUpdated, errors] = await taskService.updateOneById(
      existingBox._id,
      updateData,
    );
  ...
  it('Should not add any new tasks', async () => {
    ...
    expect(boxAfterUpdate.dailyTasks).toHaveLength(
      boxBeforeUpdate.dailyTasks.length,
    );
  });
