...
describe('DailyTaskService.addMultiple() test suite', () => {
  ...
  it('Should add tasks data to DB if input is valid', async () => {
    await taskService.addMultiple(existingBox._id, [
      taskToCreate1,
      taskToCreate2,
    ]);

    const dbResp = await boxModel
      .findById(existingBox._id)
      .select('dailyTasks');

    const clearedResp = clearDBRespDefaultFields(dbResp);

    expect(clearedResp.dailyTasks).toHaveLength(2);
    expect(clearedResp.dailyTasks).toEqual(
  ...
  it('Should return NOT_FOUND ServiceError if box does not exists', async () => {
    const [result, errors] = await taskService.addMultiple(
      getNonExisting_id(),
      [taskToCreate1, taskToCreate2],
    );
  ...
  it('Should return ServiceError REQUIRED, if the provided box _id is null', async () => {
    const [result, errors] = await taskService.addMultiple(null, [
      taskToCreate1,
      taskToCreate2,
    ]);
  ...
  it('Should return ServiceError REQUIRED, if the provided box _id is undefined', async () => {
    const [result, errors] = await taskService.addMultiple(undefined, [
      taskToCreate1,
      taskToCreate2,
    ]);
