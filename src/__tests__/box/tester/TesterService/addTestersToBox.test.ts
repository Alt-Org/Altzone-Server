...
describe('TesterService.addTestersToBox() test suite', () => {
  ...
  it('Should add testers data to the box and return true', async () => {
    const [areAdded, errors] = await service.addTestersToBox(
      existingBox._id,
      testersToAdd,
    );
    ...
    const testersInDB = boxInDB.testers.map((tester) => {
      return {
        profile_id: tester.profile_id,
        player_id: tester.player_id,
        isClaimed: tester.isClaimed,
      };
    });
