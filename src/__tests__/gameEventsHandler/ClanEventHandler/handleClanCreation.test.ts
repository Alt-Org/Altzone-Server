...
describe('ClanEventHandler.handleClanCreation() test suite', () => {
  ...
  const eventPayloadBuilder = GameEventsEmitterBuilderFactory.getBuilder(
    'GameEvent',
  ) as GameEventBuilder<'clan.create'>;
  ...
  it('Should create all UI daily tasks for the clan', async () => {
    ...
    const dailyTasks = await dailyTaskModel
      .find({ clan_id: eventPayload.info.clan_id })
      .exec();
    const uiTasks = dailyTasks.filter(
  ...
  it('Should create 20 server daily tasks for the clan', async () => {
    ...
    const dailyTasks = await dailyTaskModel
      .find({ clan_id: eventPayload.info.clan_id })
      .exec();
    const serverTasks = dailyTasks.filter(
      (task) => uiDailyTasks[task.type] == null,
    );
  ...
  it('Should throw if clan_id is not provided', async () => {
    const eventPayload = eventPayloadBuilder
      .setEventName('clan.create')
      .setInfo({ clan_id: null })
      .build() as GameEventPayload<'clan.create'>;

    const throwingCall = async () =>
      clanEventHandler.handleClanCreation(eventPayload);
