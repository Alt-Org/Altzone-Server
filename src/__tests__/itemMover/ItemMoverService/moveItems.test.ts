...
describe('ItemMoverService.moveItems() test suite', () => {
    ...
    it('Should move the items from room to specified stock', async () => {
        ...
        await itemMoverService.moveItems([item1._id, item2._id], existingStock._id, MoveTo.STOCK);

        const itemsInDB = await itemModel.find(allItemsFilter);
        const clearedItemsInDB = clearDBRespDefaultFields(itemsInDB);

        expect(clearedItemsInDB).toEqual(expect.arrayContaining([
            expect.objectContaining({...item1, stock_id: existingStock._id, room_id: null}),
            expect.objectContaining({...item2, stock_id: existingStock._id, room_id: null})
        ]));
    });
    ...
    it('Should return moved to stock items', async () => {
        ...
        const [items, errors] = await itemMoverService.moveItems([item1._id, item2._id], existingStock._id, MoveTo.STOCK);

        expect(errors).toBeNull();

        const clearedItems = clearDBRespDefaultFields(items);
        expect(clearedItems).toEqual(expect.arrayContaining([
            expect.objectContaining({...item1, room_id: existingRoom._id, stock_id: null}),
            expect.objectContaining({...item2, room_id: existingRoom._id, stock_id: null})
        ]));
    });
    ...
    it('Should move the items from stock to specified room', async () => {
        ...
        await itemMoverService.moveItems([item1._id, item2._id], existingRoom._id, MoveTo.ROOM);
    ...
    it('Should return moved to room items', async () => {
        ...
        const [items, errors] = await itemMoverService.moveItems([item1._id, item2._id], existingRoom._id, MoveTo.ROOM);
    ...
    it('Should return NOT_FOUND if no items are found', async () => {
        const [items, errors] = await itemMoverService.moveItems([getNonExisting_id()], existingRoom._id, MoveTo.ROOM);
    ...
    it('Should return NOT_FOUND if provided _ids are not mongo _ids', async () => {
        const [items, errors] = await itemMoverService.moveItems(['non-mongo-id'], existingRoom._id, MoveTo.ROOM);
