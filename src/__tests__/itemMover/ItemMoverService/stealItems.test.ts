...
describe('ItemMoverService.stealItems() test suite', () => {
    ...
    beforeEach(async () => {
        ...
        const destinationSoulHomeResp = await soulHomeModel.create(destinationSoulHome);
        ...
        stealToken = {
            exp: new Date().getTime() + 60000, iat: 1000,
            playerId: getNonExisting_id(), soulHomeId: sourceSoulHome._id
        }
    });

    it('Should move the items to specified room', async () => {
        await itemMoverService.stealItems([item1._id, item2._id], stealToken, destinationRoom._id);
    ...
    it('Should return moved items', async () => {
        const [movedItems, errors] = await itemMoverService.stealItems([item1._id, item2._id], stealToken, destinationRoom._id);
    ...
    it('Should throw NOT_FOUND ServiceError if provided items do not exist', async () => {
        const throwingCall = async () => {
            await itemMoverService.stealItems([getNonExisting_id(), getNonExisting_id()], stealToken, destinationRoom._id);
        }

        await expect(throwingCall).rejects.toThrowError(new APIError({
    ...
    it('Should throw NOT_FOUND ServiceError if provided items array is undefined', async () => {
        const throwingCall = async () => {
            await itemMoverService.stealItems(undefined, stealToken, destinationRoom._id);
        }
