...
describe('CustomCharacterService.readMany() test suite', () => {
    ...
    beforeEach(async () => {
        ...
        character1 = characterBuilder.setCharacterId(characterId).setSize(10).setPlayerId(player_id).build();
        character2 = characterBuilder.setCharacterId(characterId).setSize(20).setPlayerId(player_id).build();
        character3 = characterBuilder.setCharacterId(characterId).setSize(30).setPlayerId(player_id).build();
    ...
    it('Should return all characters that match the provided filter', async () => {
        const [characters, errors] = await characterService.readMany({ filter: allCharactersFilter });
    ...
    it('Should return all characters in DB if filter is undefined', async () => {
        const [characters, errors] = await characterService.readMany({ filter: undefined });
    ...
    it('Should return characters with only specified fields in the options.select', async () => {
        ...
        const [characters, errors] = await characterService.readMany({ filter: allCharactersFilter, select });
    ...
    it('Should limit the number of returned characters using options.limit', async () => {
        ...
        const [characters, errors] = await characterService.readMany({ filter: allCharactersFilter, limit });
    ...
    it('Should skip specified number of characters using options.skip', async () => {
        ...
        const [characters, errors] = await characterService.readMany({ filter: allCharactersFilter, skip, sort });
    ...
    it('Should return sorted characters using options.sort', async () => {
        ...
        const [characters, errors] = await characterService.readMany({ filter: allCharactersFilter, sort });

        expect(errors).toBeNull();
        expect(characters.map(characters => characters.speed)).toEqual([character1.speed, character2.speed, character3.speed]);
    });

    it('Should return characters with reference objects using options.includeRefs', async () => {
        const [characters, errors] = await characterService.readMany({ filter: allCharactersFilter, includeRefs: [ModelName.PLAYER] });

        expect(errors).toBeNull();
        characters.forEach(character => {
            const clearedPlayer = clearDBRespDefaultFields(character.Player);
            expect(clearedPlayer).toEqual(expect.objectContaining({
                ...player, _id: new ObjectId(player._id), profile_id: new ObjectId(player.profile_id)
            }));
        });
    });
    ...
    it('Should ignore non-existing references from options.includeRefs', async () => {
        ...
        const [characters, errors] = await characterService.readMany({ filter: allCharactersFilter, includeRefs: [nonExistingRef] });
